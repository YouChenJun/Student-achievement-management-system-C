...
#include<string.h>
#define N 10
const char* filename = "student.txt";//保存文件在代码所在目录
int d;//学生个数
...
void inputfile(struct student m[10])    //暂时没有实现的功能  2.0已经实现了！！
{
	char sel;//, ch[60];
	FILE* fp;
	int i, j;
	...
	if (sel == 'Y' || sel == 'y')
	{
		...
		printf("将读取%s\n若没有，则新建一个\n", filename);
		fp = fopen(filename, "w+"); //固定文件所在的目录，防止出错！w+是打开或者创建一个文本文档，可以读写

		for (i = 0; i < d; i++)
		{
			fscanf(fp, "%s %d", m[i].NAME, &m[i].num); //学生的循环
			for (j = 0; j < d; j++)
			{
